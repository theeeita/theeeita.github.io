	let yandexMap = null, $mapBlock = null, lastSelectedPoint = null;
	...
	$orderForm.find(`input[type="text"]`).add($(`.subscribe-form input[name="email"], textarea`)).on("input", function() {
		...
		if(value !== "") {
			...
			if(name === "email") {
				if(value.match(/[a-zA-Z]/g) && value.includes("@") && !value.match(/[а-яА-ЯёЁ]/g)) {
					$currentInput.removeClass("invalid-value");
					$currentInput.next().remove();
					return;
				}
				...
				if(!$currentInput.next().is(".invalid-input")) {
					$errorElement.text("Недопустимые символы. Это поле может содержать только латиницу и символ @.");
	...
	function init(coords) {
			let arrayCoords = coords.split(" ").map(item => +item);
